...
/**
 * Función que maneja los mensajes y realiza las operaciones correspondientes.
 * @param msg Todos los argumentos necesarios para realizar la operación.
 * @param callback La función de devolución de llamada que se ejecutará al finalizar la operación.
 */
export function messageHandler(
  ...
  switch (message.type) {
    case MessageType.ADD:
      console.log(
        chalk.yellow(
          "Se ha solicitado añadir una carta por parte del usuario: " +
            message.data[0],
        ),
      );
      handler.addCard(carta, (error) => {
        if (error) {
          console.log(
            chalk.red.bold("Operacion fallida por error: " + error.message),
          );
          callback(error);
          return;
        } else {
    ...
    case MessageType.REMOVE:
      console.log(
        chalk.yellow(
          "Se ha solicitado eliminar una carta por parte del usuario: " +
            message.data[0],
        ),
      );
    ...
    case MessageType.READ:
      console.log(
        chalk.yellow(
          "Se ha solicitado leer una carta por parte del usuario: " +
            message.data[0],
        ),
      );
    ...
    case MessageType.UPDATE:
      console.log(
        chalk.yellow(
          "Se ha solicitado actualizar una carta por parte del usuario: " +
            message.data[0],
        ),
      );
    ...
    case MessageType.LIST:
      console.log(
        chalk.yellow(
          "Se ha solicitado listar todas las cartas por parte del usuario: " +
            message.data[0],
        ),
      );
